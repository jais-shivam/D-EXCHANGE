...
import { selectFilledOrdersLoaded, selectOpenOrder } from "../store/exchangeSlice";
...
export const MyTransactions = () => {
  ...
  let myOpenOrders=openOrders.slice().sort((a, b) => a.timestamp - b.timestamp);
  console.log(myOpenOrders);

//   console.log('user',typeof(filledOrders[0].user));
  console.log('myAccount',typeof(myAccount));
  let myFilledOrders = filledOrders.filter((o) => (o.user === myAccount || o.userFill === myAccount));
        ...
        <Tabs defaultActiveKey="trades" className="bg-dark text-white">
          ...
          <Tab eventKey="orders" title="Orders">
            <table className="table table-dark table-sm small">
              ...
              { myOpenOrders ? showMyOpenOrders(myOpenOrders) : <Spinner type="table" />}
            </table>
          </Tab>
        </Tabs>
      </div>
